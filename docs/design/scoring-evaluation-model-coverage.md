...
| Catálogo M-C01–M-R04 | **Implementado (calculables)** | `build_metrics()` itera `METRIC_SPECS` y cablea las 10 métricas con evidencia disponible (M-C01/02/03, M-T01/02/03/04, M-O04, M-R01/02/04). Quedan fuera solo las no calculables (M-O01–O03, M-R03, M-C04, M-C05). |
...
| Flags bloqueantes | **Parcial** | Se bloquea por `session_failed`, ausencia de `session_completed`, objetivo no completado (M-C03) y error técnico no recuperado (M-R02). Falta solo la regla de herramienta crítica sin resolución (M-R03), bloqueada por eventos fuente. |
...
| Densidad de uso de herramientas | M-O04 | Operational | `tool_usage_density` | `×100` → informativa | 0 |
...
Notas de diseño relevantes: las cinco métricas-tasa aplican `×100` antes de normalizar porque las evidencias son ratios en `[0,1]`; M-T04 y M-O04 quedan con **peso 0** (informativas: aparecen en el informe pero no entran en la media ponderada, al no existir umbral de referencia en el doc 3.3). La cobertura y los bloqueos de cada métrica fuente están detallados en [evidence-coverage-by-quality-dimension.md](./evidence-coverage-by-quality-dimension.md).
...
| Herramienta crítica fallida sin resolución (M-R03) | **Bloqueado** | Faltan `tool.failed`, `tool.timeout` y `tool.response_received`; además no existe configuración de criticidad por herramienta. Véase [vapi-event-coverage.md](./vapi-event-coverage.md). |
...
| Conteos de turnos, eventos y herramientas | No | No | **Pendiente.** No forman parte de `EvaluationReport`. |
...
El cableado del catálogo (M-C01–M-R04 calculables) y los flags de M-C03/M-R02 **ya están implementados** en el cambio `scoring-wire-evidences`, con tests de normalización, pesos, reescala `×100`, exclusión de métricas ausentes y de peso 0, y prueba de que una evidencia nueva mueve la nota global. Pendiente, en este orden:

1. definir pesos y umbrales configurables por agente (la tabla `METRIC_SPECS` es su precursor);
2. revisar los umbrales inventados por falta de referencia en el doc 3.3: par óptimo/degradado de M-T02 (3,0/5,0 s) y el tratamiento informativo (peso 0) de M-T04 y M-O04;
3. desbloquear M-R03 (herramienta crítica sin resolución) cuando existan `tool.failed`/`tool.timeout`/`tool.response_received`;
4. cerrar el enlace `Metric → Evidence → event_id` y exponerlo en el informe;
5. ampliar el informe con incidencias, cobertura evaluada y oportunidades de mejora;
6. corregir el orden del pipeline para que las señales post-terminales participen en la primera evaluación útil.
